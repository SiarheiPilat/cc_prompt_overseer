...
export function CalendarGrid({ year, month, data }: { year: number; month: number; data: Array<{ d: string; n: number }> }) {
        ...
        {days.map((d, i) => {
          ...
          return (
            <Link key={d.d}
              ...
              <div className="mt-auto text-[10px] text-mutedfg tabular-nums">{d.n || ""}</div>
            </Link>
          );
        })}
      </div>
